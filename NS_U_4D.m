...
% LU = 0.1;%lunghezza di riferimento
LU = 1;
%omega0=1.6853/LU;
omega0=7.384/LU;
...
equilib = true;  % L'ho messo io per il calcolo di tutto
% X0=[0.01;0.01]; % initial condition for time integration
X0 = [0.01; 0.01; 0.01; 0.01]; %initial condition 4D model

nodes=[21524];  % a node at end of cantilever
%nodes=[1696];
FRCplot=1;

% out_name='./results/cylinder_before20';
out_name='./results/Uchannel1';
% nameode='cylinder_before20';
nameode='Uchannel_auto';% name of ode function
% f2use=@cylinder_before20;   % ode file to use in ./models
f2use=@Uchannel_auto;
% namefile='../output/_cylinder_before20/param';
namefile='C:\Users\Mattia\Desktop\documenti double degree\esami dd\stages\ensta\DPIM_NS 2\output\param.mat';
% nu0=coeffnorm*LU/20.0;
nu0=coeffnorm*LU/4700;

nu=nu0+0.0001;

par_nu=nu;   %AGGIUNTA NON SO SE SIA GIUSTO!!!!!!!!!!!!!
...
figure(10)   %LIMIT CYCLE WITH NU PARAMETER
...
period=2*pi/omega0;
[t,y]=ode113(hls{2},linspace(0,period,320),x1,options,par_nu);  % one period
plot(t,y(:,1)) % activate to plot
...
opt=contset(opt,'MaxNumPoints',1200); % how many points in FRF
opt=contset(opt,'InitStepsize',4e-7); % initial dimension of continuation arch
opt=contset(opt,'MaxStepsize',9e-3); % max dimension of continuation arch
